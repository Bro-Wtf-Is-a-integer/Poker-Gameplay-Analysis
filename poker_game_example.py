...
class GameState(object):
    ...
    def dealing_cards_fixed(self):
        index = self.nn_current_hand % len(pe_.fixed_hands)
        self.agent.current_hand = pe_.fixed_hands[index][0]
        self.agent.evaluate_hand()
        self.opponent.current_hand = pe_.fixed_hands[index][1]
        self.opponent.evaluate_hand()
...
def get_next_states(last_state):
    """debugg purpose to see how many states has been generated, 
    there has been cases where this function is in an infinite loop"""

    #print(f"Generating next states for hand {last_state.nn_current_hand}, Agent Stack: {last_state.agent.stack}")
...
agent = PokerPlayer(current_hand_=None, stack_=400, action_=None, action_value_=None)
opponent = PokerPlayer(current_hand_=None, stack_=400, action_=None, action_value_=None)
...
init_state = GameState(nn_current_hand_=0,
                       ...
                       phase_ = 'INIT_DEALING',
                       pot_=0,
                       acting_agent_=None, #changing to agent
                       agent_=agent,
                       opponent_=opponent,
                       )
